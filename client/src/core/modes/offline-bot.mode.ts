...
export default class BotGameMode extends EventTarget implements IGameMode {
  public readonly title: string = "Bot Mode";
  private readonly room: IRoom;
  private readonly positions: (IPlayerCharacter | undefined)[] = [];
  private turn: number = 0;
  private ready = false;
  private over = false;
  private gameOverStatus: GameOverStatus | undefined;
  ...
  public constructor() {
    ...
    const capability = (instance: IPlayer) => ({
      takePosition: async (index: number) => {
        const ok = await this.takePosition(index, await instance.character);
    ...
    const p1 = new Bot("bot", capability);
  ...
  private addPlayer(player: IPlayer): Promise<void> {
  ...
  private async getGameOverStatus(): Promise<GameOverStatus> {
